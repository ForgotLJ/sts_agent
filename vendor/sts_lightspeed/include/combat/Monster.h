...
//        bool isDyingB = false;
    ...
    void Monster::addDebuff(int amount, bool isSourceMonster) {
        if constexpr (s == MS::WEAK || s == MS::VULNERABLE) {
            if (isSourceMonster) {
    ...
    void Monster::setJustApplied(bool value) {
        static_assert(s == MS::VULNERABLE || s == MS::WEAK || s == MS::RITUAL,
                      "status does not use the just-applied bitset");
        constexpr std::uint64_t mask = s == MS::VULNERABLE ? 0x1ULL << 63
                                     : s == MS::WEAK ? 0x1ULL << 62
                                                     : 0x1ULL << 61;
