...
// intangible
    ...
    void Player::setJustApplied(bool value) {
        static_assert(s > PS::INVALID && s <= PS::WEAK, "status does not use the just-applied bitset");
        constexpr auto index = static_cast<unsigned>(s);
        if (value) {
            justAppliedBits |= (1ULL << index);
        } else {
            justAppliedBits &= ~(1ULL << index);
        }
    }
    ...
    bool Player::wasJustApplied() const {
        static_assert(s > PS::INVALID && s <= PS::WEAK, "status does not use the just-applied bitset");
        return justAppliedBits & (1ULL << static_cast<unsigned>(s));
    }
...
// intangible
    ...
    void Player::buff(int amount) {
        ...
        if constexpr (s == PS::DOUBLE_DAMAGE || s == PS::INTANGIBLE) { // todo this is definitely wrong
    ...
    void Player::debuff(int amount, bool isSourceMonster) {
        ...
        if constexpr (s == PS::WEAK || s == PS::FRAIL || s == PS::VULNERABLE || s == PS::DRAW_REDUCTION) {
    ...
    void Player::setHasRelic(bool value) {
        static_assert(static_cast<unsigned>(r) < 128, "battle relic bit index is out of range");
        if (value) {
            if constexpr (static_cast<unsigned>(r) < 64) {
                relicBits0 |= 1ULL << static_cast<unsigned>(r);
            } else {
                relicBits1 |= 1ULL << (static_cast<unsigned>(r) - 64);
            }
        } else {
            if constexpr (static_cast<unsigned>(r) < 64) {
                relicBits0 &= ~(1ULL << static_cast<unsigned>(r));
    ...
    bool Player::hasRelic() const {
        static_assert(static_cast<unsigned>(r) < 128, "battle relic bit index is out of range");
        if constexpr (static_cast<unsigned>(r) < 64) {
            return relicBits0 & (1ULL << static_cast<unsigned>(r));
        } else {
            return relicBits1 & (1ULL << (static_cast<unsigned>(r) - 64));
        }
    }
