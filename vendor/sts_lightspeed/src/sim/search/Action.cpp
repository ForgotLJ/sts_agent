...
std::ostream& search::Action::printDesc(std::ostream &os, const BattleContext &bc) const {
    ...
    switch (getActionType()) {
        ...
        case ActionType::POTION: {
            ...
            const bool isDiscard = getTargetIdx() > 5;
            os << "{ ";
            if (isDiscard) {
                os << "discard potion ";
            } else {
            ...
            if (!isDiscard && potionRequiresTarget(p)) {
                const auto &m = bc.monsters.arr[getTargetIdx()];
                os << " -> (" << getTargetIdx() << ") " << m.getName();
            }
            return os << " }";
...
std::vector<search::Action> search::Action::getAllActionsInState(const BattleContext &bc) {
    ...
    if (bc.inputState == InputState::CARD_SELECT) {
        for (const auto action : enumerateCardSelectActions(bc)) {
        ...
        for (int handIdx = 0; handIdx < bc.cards.cardsInHand; ++handIdx) {
            ...
            if (card.requiresTarget()) {
                for (int targetIdx = 0; targetIdx < bc.monsters.monsterCount; ++targetIdx) {
                    ...
                    const search::Action action(ActionType::CARD, handIdx, targetIdx);
                    if (action.isValidAction(bc)) {
                        actions.push_back(action);
                    }
                }
            } else {
                const search::Action action(ActionType::CARD, handIdx);
                if (action.isValidAction(bc)) {
                    actions.push_back(action);
                }
            }
    ...
    for (int potionIdx = 0; potionIdx < bc.potionCapacity; ++potionIdx) {
        ...
        if (potion == Potion::INVALID || potion == Potion::EMPTY_POTION_SLOT) {
            continue;
        }

        const search::Action discardAction(ActionType::POTION, potionIdx, -1);
        if (discardAction.isValidAction(bc)) {
            actions.push_back(discardAction);
        }

        if (potion == Potion::FAIRY_POTION) {
            continue;
        }

        if (!potionRequiresTarget(potion)) {
            const search::Action useAction(ActionType::POTION, potionIdx);
            if (useAction.isValidAction(bc)) {
                actions.push_back(useAction);
            }
            continue;
        }
        ...
        for (int targetIdx = 0; targetIdx < bc.monsters.monsterCount; ++targetIdx) {
            ...
            const search::Action useAction(ActionType::POTION, potionIdx, targetIdx);
            if (useAction.isValidAction(bc)) {
                actions.push_back(useAction);
            }
        }
