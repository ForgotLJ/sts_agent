...
GameContext::GameContext(
    ...
    cc(cc),
    map(new Map(Map::fromSeed(seed, ascension, 1, finalActUnlocked))),
    ascension(ascension) {
    if (actOneBossesSeen < 0 || actOneBossesSeen > 3) {
        throw std::invalid_argument("act one bosses seen must be between 0 and 3");
    }
    ...
    generateMonsters(actOneBossesSeen);
    initRelics();
    initPlayer();
...
void GameContext::generateMonsters(int actOneBossesSeen) {
    generateWeakMonsters();
    generateStrongMonsters();
    generateElites();
    generateBoss(actOneBossesSeen);
}
...
void GameContext::generateBoss(int actOneBossesSeen) {
    ...
    if (act == 1 && actOneBossesSeen < 3) {
        boss = bosses[0][actOneBossesSeen];
        return;
    }
