...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate M7-B teacher-action agreement by non-combat phase."
    )
...
def main() -> int:
    ...
    if args.device.startswith("cuda") and not torch.cuda.is_available():
        raise RuntimeError("M7-B imitation evaluation requested unavailable CUDA")
    ...
    if args.replay_cache is not None:
        replay_cache = verify_m7b_replay_manifest(
            args.replay_cache,
            expected_corpus_sha256=corpus["aggregate_sha256"],
            expected_encoder_config=loaded.trainer.encoder.config.to_dict(),
            expected_chunk_length=args.chunk_length,
            expected_burn_in_steps=args.burn_in_steps,
        )
    metrics = evaluate_m7b_imitation(
        loaded.trainer,
        LightspeedEnvironmentFactory(),
        corpus_trace_paths(corpus),
        trace_batch_size=args.trace_batch_size,
        optimizer_batch_chunks=args.optimizer_batch_chunks,
        chunk_length=args.chunk_length,
        burn_in_steps=args.burn_in_steps,
        replay_batch_paths=(
            None if replay_cache is None else replay_cache_batch_paths(replay_cache)
        ),
    )
    payload = {
        ...
        "replay_cache_sha256": (
            None if replay_cache is None else replay_cache["aggregate_sha256"]
        ),
