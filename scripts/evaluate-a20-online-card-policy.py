...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Paired Lightspeed evaluation for an Ironclad online card-reward cold start."
    )
...
def main() -> int:
    ...
    if args.max_steps <= 0 or args.bootstrap_samples <= 0:
        raise ValueError("evaluation limits must be positive")
    checkpoint = args.checkpoint.resolve()
    if not checkpoint.is_file():
        raise FileNotFoundError(checkpoint)
    model = load_online_card_ranker(checkpoint, args.device)
    ...
    candidate_summary = evaluate_full_runs(
        environment_factory,
        lambda _, __: A20OnlineCardRewardPolicy(model),
        seeds,
    ...
    payload: dict[str, Any] = {
        "protocol": "a20-online-card-reward-paired-lightspeed-evaluation",
        "schema_version": 1,
